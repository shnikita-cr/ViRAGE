...
class VisRAGService(BaseService):
    def invoke(
        ...
        if retrieved.backend_name == "local_tfidf":
            caveats.append(
                "Semantic retrieval is running on the local TF-IDF backend; enable dense embeddings for stronger recall.")
    ...
    def _build_visualization_plan(
        ...
        visual_task = query_understanding.requested_operations[
            0] if query_understanding.requested_operations else "exploratory analysis"
        ...
        field_bindings, transforms, axes, build_instructions, mark_hints, renderer_hints = self._plan_channels(
            chart_family, data_profile, request_analysis)
    ...
    def _build_candidate_spec_set(
        ...
        candidate_specs = [
            CandidateSpec(
                ...
                visualization_plan=(plan.model_copy(
                    update={"chart_family": rec.chart_family}) if rec.chart_family != plan.chart_family else plan),
            )
    ...
    def _plan_channels(
        ...
        list[VisualizationFieldBinding], list[VisualizationTransform], list[VisualizationAxisInstruction], list[str],
        list[str], list[str]]:
        numeric = [col for col in request_analysis.selected_fields if
                   col in data_profile.likely_numeric_columns] or list(data_profile.likely_numeric_columns)
        categorical = [col for col in request_analysis.selected_fields if
                       col in data_profile.likely_categorical_columns] or list(data_profile.likely_categorical_columns)
        temporal = [col for col in request_analysis.selected_fields if col in data_profile.likely_time_columns] or list(
            data_profile.likely_time_columns)
        ...
        def bind(channel: str, field_name: str, field_role: str, *, aggregate: str | None = None,
                 title: str | None = None, time_unit: str | None = None) -> None:
            encodings.append(VisualizationFieldBinding(channel=channel, field_name=field_name, field_role=field_role,
                                                       title=title or field_name, aggregate=aggregate,
                                                       time_unit=time_unit))
        ...
        if chart_family == "line":
            ...
            axes.extend([
                VisualizationAxisInstruction(channel="x", field_name=x_field, title=x_field,
                                             scale_type="temporal" if temporal else "categorical",
                                             rotate_labels=bool(temporal or categorical)),
                VisualizationAxisInstruction(channel="y", field_name=y_field, title=y_field, scale_type="linear"),
            ])
            transforms.append(
                VisualizationTransform(kind="aggregate", field_name=y_field, aggregate="mean", group_by=[x_field],
                                       description="Aggregate the numeric measure by the x axis before plotting when duplicate x values appear."))
            instructions.extend([
            ...
            renderer_hints.extend(
                ["Rotate x-axis labels if dates are dense.", "Tighten layout after formatting dates."])
            ...
            instructions.extend([f"Use '{x_field}' on the x-axis and '{y_field}' on the y-axis.",
            ...
            transforms.append(VisualizationTransform(kind="bin", field_name=x_field,
                                                     description="Bin the numeric values before counting observations."))
            transforms.append(
                VisualizationTransform(kind="aggregate", field_name="count", aggregate="count", group_by=[x_field],
                                       description="Count observations per histogram bin."))
            instructions.extend([f"Use '{x_field}' as the quantitative field for the histogram bins.",
                                 "Count observations per bin on the y-axis."])
            ...
            bind("y", y_field, "quantitative")
            axes.extend([
                VisualizationAxisInstruction(channel="x", field_name=x_field, title=x_field, scale_type="categorical",
                                             rotate_labels=True),
                VisualizationAxisInstruction(channel="y", field_name=y_field, title=y_field, scale_type="linear"),
            ])
            instructions.extend([f"Group the distribution by '{x_field}'.",
                                 f"Use '{y_field}' as the quantitative spread shown by the boxplot."])
            ...
            axes.extend([
                VisualizationAxisInstruction(channel="x", field_name=x_field, title=x_field,
                                             scale_type="categorical" if categorical else "temporal",
                                             rotate_labels=True),
                VisualizationAxisInstruction(channel="y", field_name=y_field, title=y_field, scale_type="linear"),
            ])
            transforms.append(
                VisualizationTransform(kind="aggregate", field_name=y_field, aggregate="mean", group_by=[x_field],
                                       description="Aggregate the measure by category before plotting bars."))
            instructions.extend([f"Use '{x_field}' as the discrete comparison axis.",
                                 f"Use '{y_field}' as the quantitative bar height after aggregation."])
    ...
    def _normalize_candidate_charts(candidate_charts: list[str]) -> tuple[list[str], list[str]]:
        ...
        for raw_chart in candidate_charts:
            ...
            if not chart and raw in _FALLBACK_CHART_MAP:
                ...
                notes.append(
                    f"Chart family '{raw}' is not directly supported by the current renderer; using '{chart}' instead.")
            elif not chart:
    ...
    @staticmethod
    def _instruction_highlights(chart_type: str, data_profile: DataProfile, request_analysis: RequestAnalysisResult) -> \
    list[str]:
        selected = request_analysis.selected_fields
        if chart_type == "line":
            x = next((field for field in selected if field in data_profile.likely_time_columns), None) or (
                data_profile.likely_time_columns[0] if data_profile.likely_time_columns else "index")
            y = next((field for field in selected if field in data_profile.likely_numeric_columns), None) or (
                data_profile.likely_numeric_columns[0] if data_profile.likely_numeric_columns else "value")
