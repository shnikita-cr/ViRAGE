...
class VisRAGService(BaseService):
    def invoke(self, query_understanding: QueryUnderstandingResult, request_analysis: RequestAnalysisResult, data_profile: DataProfile, runtime: RuntimeContext) -> VisRAGResult:
        ...
        recommendations = self._build_recommendations(query_understanding=query_understanding, request_analysis=request_analysis, data_profile=data_profile, candidate_charts=candidate_charts, support=support, top_k=runtime.settings.visrag_top_k_recommendations)
        plan = self._build_visualization_plan(query_understanding=query_understanding, request_analysis=request_analysis, data_profile=data_profile, recommendations=recommendations, support=support)
        candidate_spec_set = self._build_candidate_spec_set(recommendations, retrieved["examples"], plan)
        caveats = self._dedupe([*query_understanding.constraints, *query_understanding.ambiguity_notes, *request_analysis.ambiguity_report, *request_analysis.missing_fields, *normalization_notes])
        ...
        if runtime.settings.visrag_enable_llm_synthesis:
            ...
            plan, llm_caveats = self._refine_plan_with_llm(plan, query_understanding, retrieved["examples"], reasoning_llm, runtime)
        ...
        return VisRAGResult(recommendations=recommendations, visualization_plan=plan, rules=["Prefer specifications that map cleanly to Vega-Lite encodings and transforms.", "Keep titles, axes and marks explicit so downstream generation stays deterministic.", "Prefer retrieved corpus examples when they agree with the detected data shape."], caveats=self._dedupe(caveats), implementation_notes=plan.build_instructions, retrieved_examples=[item.to_model() for item in retrieved["examples"]], corpus_status=retrieved["corpus_status"], retrieval_strategy=(f"multi_query_{runtime.settings.visrag_embedding_backend}" if retrieved["examples"] else "heuristic_only"), retrieval_query=" || ".join(retrieval_queries), candidate_spec_set=candidate_spec_set)

    def _build_retrieval_queries(self, query_understanding: QueryUnderstandingResult, request_analysis: RequestAnalysisResult, data_profile: DataProfile) -> list[str]:
    ...
    def _retrieve_multi_query(self, queries: list[str], candidate_charts: list[str], runtime: RuntimeContext) -> dict[str, Any]:
        merged: dict[str, Any] = {}
        corpus_status: dict[str, str] = {}
        for query_text in queries:
            bundle = retrieve_examples(runtime.settings.visrag_corpus_root, index_root=runtime.settings.visrag_index_root, query_text=query_text, preferred_chart_types=candidate_charts, top_k=runtime.settings.visrag_top_k_examples, fetch_k=runtime.settings.visrag_retriever_fetch_k, similarity_threshold=runtime.settings.visrag_similarity_threshold, embedding_backend=runtime.settings.visrag_embedding_backend, embedding_model=runtime.settings.visrag_embedding_model, ollama_base_url=runtime.settings.visrag_ollama_base_url, ollama_timeout_seconds=runtime.settings.visrag_ollama_timeout_seconds, force_rebuild=runtime.settings.visrag_force_rebuild_index)
    ...
    def _build_recommendations(self, *, query_understanding: QueryUnderstandingResult, request_analysis: RequestAnalysisResult, data_profile: DataProfile, candidate_charts: list[str], support: dict[str, list[Any]], top_k: int) -> list[VisRAGRecommendation]:
        ...
        for index, chart_type in enumerate(chart_pool):
            heuristic_score, heuristic_reason = self._heuristic_score(chart_type=chart_type, index=index, query_understanding=query_understanding, request_analysis=request_analysis, data_profile=data_profile)
        ...
        return [VisRAGRecommendation(chart_family=chart_type, rationale=rationale, priority=priority, score=score, support_examples=support_examples, instruction_highlights=instruction_highlights) for priority, (chart_type, score, rationale, support_examples, instruction_highlights) in enumerate(scored[:top_k], start=1)]

    def _build_visualization_plan(self, *, query_understanding: QueryUnderstandingResult, request_analysis: RequestAnalysisResult, data_profile: DataProfile, recommendations: list[VisRAGRecommendation], support: dict[str, list[Any]]) -> VisualizationPlan:
        if not recommendations:
            raise RuntimeError("VisRAG could not produce any recommendations.")
        top = recommendations[0]
        chart_family = top.chart_family
        x_field, x_role, y_field, y_role, color_field, color_role = self._plan_channels(chart_family, data_profile, request_analysis)
        bindings = [VisualizationFieldBinding(channel="x", field_name=x_field, field_role=x_role, title=x_field.title(), required=True), VisualizationFieldBinding(channel="y", field_name=y_field, field_role=y_role, title=y_field.title(), aggregate="mean" if chart_family in {"line", "bar", "area"} else None, required=True)]
        axes = [VisualizationAxisInstruction(channel="x", field_name=x_field, title=x_field.title(), scale_type=("temporal" if x_role == "temporal" else "linear" if x_role == "quantitative" else "categorical"), rotate_labels=x_role == "temporal"), VisualizationAxisInstruction(channel="y", field_name=y_field, title=y_field.title(), scale_type="linear")]
        if color_field:
            bindings.append(VisualizationFieldBinding(channel="color", field_name=color_field, field_role=color_role, title=color_field.title(), required=False))
        transforms = []
        if chart_family == "histogram":
            transforms.append(VisualizationTransform(kind="bin", field_name=x_field, description=f"Bin {x_field} into histogram buckets."))
        build_instructions = [*top.instruction_highlights, "Use explicit encodings so the final Vega-Lite spec remains deterministic."]
        evidence_ids = [item.example_id for item in support.get(chart_family, [])[:3]]
        return VisualizationPlan(chart_family=chart_family, visual_task=query_understanding.task_type or query_understanding.intent, goal=query_understanding.user_goal or query_understanding.intent, title=query_understanding.intent, description=top.rationale, field_bindings=bindings, transforms=transforms, axes=axes, build_instructions=self._dedupe(build_instructions), mark_hints=[chart_family], renderer_hints=["Prefer readable defaults."], evidence_example_ids=evidence_ids, confidence=max(0.01, min(1.0, top.score)), vega_lite_ready=True)

    def _build_candidate_spec_set(self, recommendations: list[VisRAGRecommendation], retrieved_examples: list[Any], plan: VisualizationPlan) -> CandidateSpecSet:
        specs = []
        for index, item in enumerate(recommendations, start=1):
            ...
            specs.append(CandidateSpec(spec_id=f"candidate-{index:02d}-{item.chart_family}", chart_family=item.chart_family, summary=item.rationale, score=item.score, rationale=item.rationale, visualization_plan=candidate_plan))
        return CandidateSpecSet(candidate_specs=specs, retrieved_examples=[example.to_model() for example in retrieved_examples], visualization_plan=plan, ranking_hints=[item.rationale for item in recommendations], selected_candidate_spec=(specs[0] if specs else None))

    def _plan_channels(self, chart_family: str, data_profile: DataProfile, request_analysis: RequestAnalysisResult) -> tuple[str, str, str, str, str | None, str]:
        ...
        if chart_family in {"line", "area"} and time_columns and numeric_columns:
            color_field = next((field for field in selected if field in categorical_columns), categorical_columns[0] if categorical_columns else None)
    ...
    def _heuristic_score(self, *, chart_type: str, index: int, query_understanding: QueryUnderstandingResult, request_analysis: RequestAnalysisResult, data_profile: DataProfile) -> tuple[float, str]:
        query_text = " ".join([query_understanding.intent, *query_understanding.requested_operations, *request_analysis.grounded_fields]).lower()
        ...
        if chart_type in {"bar", "boxplot", "tick"} and data_profile.likely_categorical_columns and data_profile.likely_numeric_columns:
    ...
    def _instruction_highlights(self, chart_type: str, data_profile: DataProfile, request_analysis: RequestAnalysisResult) -> list[str]:
        if chart_type in {"line", "area"}:
            x = data_profile.likely_time_columns[0] if data_profile.likely_time_columns else (request_analysis.selected_fields[0] if request_analysis.selected_fields else "date")
    ...
    def _refine_plan_with_llm(self, plan: VisualizationPlan, query_understanding: QueryUnderstandingResult, retrieved_examples: list[Any], reasoning_llm: Any, runtime: RuntimeContext) -> tuple[VisualizationPlan, list[str]]:
        example_text = "\n".join(f"- {item.example_id} | {item.chart_type} | {item.instruction} | score={item.score:.3f}" for item in retrieved_examples[:3]) or "- none"
        prompt = (
        ...
        parsed = invoke_structured(reasoning_llm, prompt, _PlanRefinementSchema, runtime=runtime, stage="visrag_refinement", role="reasoning", examples=[{"title": plan.title, "subtitle": plan.subtitle or "", "description": plan.description or plan.goal, "build_instructions": plan.build_instructions[:2] or ["Keep encodings explicit."], "mark_hints": plan.mark_hints[:2] or [plan.chart_family], "renderer_hints": plan.renderer_hints[:2] or ["Prefer readable defaults."], "extra_caveats": []}], max_attempts=2)
    ...
    def _normalize_candidate_charts(candidate_charts: list[str]) -> tuple[list[str], list[str]]:
        ...
        for raw_chart in candidate_charts:
            ...
            chart = canonicalize_chart_type(raw) or _FALLBACK_CHART_MAP.get(raw, raw if raw in _SUPPORTED_PIPELINE_CHARTS else "")
