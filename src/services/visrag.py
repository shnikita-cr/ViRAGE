...
class VisRAGService(BaseService):
    def invoke(
        ...
        retrieval_query = self._build_retrieval_query(query_understanding, planning, data_profile)
        ...
        if not retrieved.examples:
            caveats.append("No local corpus matches were retrieved; VisRAG used data-aware fallback planning.")
        if retrieved.backend_name == "local_tfidf":
            caveats.append("Semantic retrieval is running on the local TF-IDF fallback backend; enable Ollama embeddings for denser retrieval.")

        if runtime.llm is not None and runtime.settings.visrag_enable_llm_synthesis and is_langchain_available():
            plan, llm_caveats = self._refine_plan_with_llm(plan=plan, query_understanding=query_understanding, retrieved_examples=retrieved.examples, runtime=runtime)
    ...
    def _build_recommendations(
        ...
        *,
        ...
        for index, chart_type in enumerate(chart_pool):
            ...
            instruction_highlights = self._instruction_highlights(chart_type, data_profile)
            ...
            scored.append(
                (chart_type, total_score, " ".join(rationale_parts), [item.example_id for item in evidence[:3]], instruction_highlights)
            )
    ...
    def _build_visualization_plan(
        ...
        *,
        ...
        subtitle = f"Mode: {planning.mode.value}; preferred chart family: {chart_family}."
        ...
        return VisualizationPlan(
            ...
            build_instructions=self._dedupe(build_instructions + self._planning_instructions(planning)),
    ...
    def _plan_channels(self, *, chart_family: str, data_profile: DataProfile) -> tuple[list[VisualizationFieldBinding], list[VisualizationTransform], list[VisualizationAxisInstruction], list[str], list[str], list[str]]:
        numeric = list(data_profile.likely_numeric_columns)
        categorical = list(data_profile.likely_categorical_columns)
        temporal = list(data_profile.likely_time_columns)
        ...
        if chart_family == "line":
            ...
            transforms.append(VisualizationTransform(kind="aggregate", field_name=y_field, aggregate="mean", group_by=[x_field], description="Aggregate the numeric measure by the temporal or ordered x axis before plotting."))
            instructions.extend([
                f"Bind the x-axis to '{x_field}' and treat it as {'temporal' if temporal else 'ordered categorical'}.",
            ...
            bind("y", y_field, "quantitative")
            if categorical:
                bind("color", categorical[0], "nominal", title=categorical[0])
            axes.extend([
                VisualizationAxisInstruction(channel="x", field_name=x_field, title=x_field, scale_type="linear"),
                VisualizationAxisInstruction(channel="y", field_name=y_field, title=y_field, scale_type="linear"),
            ])
            instructions.extend([
                f"Use '{x_field}' on the x-axis and '{y_field}' on the y-axis.",
            ...
            instructions.extend([
                f"Use '{x_field}' as the quantitative field for the histogram bins.",
            ...
            instructions.extend([
                f"Group the distribution by '{x_field}'.",
            ...
            instructions.extend([
                f"Use '{x_field}' as the discrete comparison axis.",
    ...
    def _heuristic_score(
        ...
        *,
        ...
        if planning.mode is ChartCaseType.NON_CANONICAL:
            base -= 0.05
        if chart_type == "line":
            ...
            return base + 0.15, "Line retained as a trend fallback."
        if chart_type == "scatter":
            ...
            return base + 0.10, "Scatter retained as a weak relationship fallback."
    ...
    @staticmethod
    def _build_retrieval_query(query_understanding: QueryUnderstandingResult, planning: PlanningResult, data_profile: DataProfile) -> str:
        parts: list[str] = [
            ...
            f"planning_mode: {planning.mode.value}",
            f"planning_steps: {', '.join(step.name for step in planning.steps)}",
        ]
    ...
    def _normalize_candidate_charts(candidate_charts: list[str]) -> tuple[list[str], list[str]]:
        ...
        for raw_chart in candidate_charts:
            ...
            if not chart and raw in _FALLBACK_CHART_MAP:
                ...
                notes.append(f"Chart family '{raw}' is not directly supported by the current renderer; using '{chart}' fallback.")
            elif not chart:
    ...
    @staticmethod
    def _planning_instructions(planning: PlanningResult) -> list[str]:
        instructions = [f"Planning step: {step.description}" for step in planning.steps[:3]]
        instructions.extend([f"Success criterion: {criterion}" for criterion in planning.success_criteria[:2]])
        return instructions

    @staticmethod
    def _instruction_highlights(chart_type: str, data_profile: DataProfile) -> list[str]:
        if chart_type == "line":
            x = data_profile.likely_time_columns[0] if data_profile.likely_time_columns else "index"
    ...
    def _refine_plan_with_llm(
        ...
        *,
        ...
        try:
            parsed = invoke_structured(runtime.llm, prompt, _PlanRefinementSchema)
        except Exception:
            return plan, ["LLM synthesis failed; VisRAG kept the deterministic visualization plan."]
