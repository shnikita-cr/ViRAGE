...
class HybridAnalysisEvaluator:
    ...
    def evaluate(
        ...
        if self.mode in {"llm", "hybrid"} and runtime is not None and runtime.reasoning_llm is not None:
            try:
                ...
                return AnalysisEvaluationResult(
                    mode=f"{self.mode}_rules_recovery",
                    ...
                    rationale=f"LLM evaluation failed: {type(exc).__name__}: {exc}. Rule recovery: {rule_result.rationale}",
                )
        return rule_result
    ...
    def _evaluate_by_llm(
            ...
            *,
            ...
            rule_baseline: AnalysisEvaluationResult,
    ) -> AnalysisEvaluationResult:
        prompt = (
        ...
        return AnalysisEvaluationResult(
            ...
            exact_match=rule_baseline.exact_match,
            numeric_match_rate=rule_baseline.numeric_match_rate,
            expected_items_count=rule_baseline.expected_items_count,
            matched_items_count=rule_baseline.matched_items_count,
