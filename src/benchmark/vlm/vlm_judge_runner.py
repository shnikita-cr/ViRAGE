...
class VLMJudgeBenchmarkRunner:
    ...
    def run_case(self, *, case: BenchmarkCase, case_root: Path, output_dir: Path) -> VLMJudgeBenchmarkResult:
        ...
        try:
            ...
            if not validation.is_valid:
                validation = self._recover_reference_validation(spec, validation)
    ...
    @staticmethod
    def _recover_reference_validation(spec: dict[str, Any], validation: SpecValidationResult) -> SpecValidationResult:
        # Ground-truth specs from public corpora can contain schema shortcuts that are renderable even when the project
        # field validator is stricter. This recovery step lets the judge benchmark render the reference image when the source spec remains renderable.
