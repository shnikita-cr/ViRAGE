...
class VegaChatBenchmarkRunner:
    ...
    def run_case(self, *, case: BenchmarkCase, case_root: Path, output_dir: Path) -> BenchmarkCaseResult:
        ...
        try:
            ...
            failed = BenchmarkCaseResult(
                ...
                reference_selection_method=None,
                reference_render_error_count=0,
                reference_render_error_rate=0.0 if case.effective_reference_specs() else None,
                is_valid_spec=False,
                is_empty_chart=True,
    ...
    def _write_report(report: BenchmarkAggregateReport, output_dir: Path) -> None:
        ...
        lines = [
            ...
            f"Repeat steps rate: {report.repeat_steps_rate}",
            f"Repeat steps rate (success): {report.repeat_steps_rate_success}",
            f"Mean attempts (success): {report.mean_attempts_success}",
            f"Mean technical attempts (success): {report.mean_technical_attempts_success}",
            f"Mean semantic attempts (success): {report.mean_semantic_attempts_success}",
