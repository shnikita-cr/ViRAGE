...
class PipelineNodes:
    ...
    def query_understanding_node(self, state: PipelineState) -> dict:
        ...
        return {"query_understanding": result, "case_type": result.case_type,
    ...
    def planning_canonical_node(self, state: PipelineState) -> dict:
        result = self.planning.invoke(state["query_understanding"], runtime=self.runtime)
    ...
    def planning_non_canonical_node(self, state: PipelineState) -> dict:
        result = self.planning.invoke(state["query_understanding"], runtime=self.runtime)
        return {"planning": result, "stage": PipelineStage.PLANNING,
    ...
    def codegen_node(self, state: PipelineState) -> dict:
        result = self.codegen.invoke(state["query_understanding"], state["data_profile"], state["data_preparation"],
                                     state["visrag"], state["run_id"], runtime=self.runtime)
    ...
    def artifact_store_node(self, state: PipelineState) -> dict:
        result = self.artifact_store.invoke(state["data_preparation"], state["execution"], state["run_id"],
                                            runtime=self.runtime)
        return {"artifact_bundle": result, "stage": PipelineStage.ARTIFACT_STORE,
