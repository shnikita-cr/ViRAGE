...
def build_tree(root_dir: Union[str, Path], extensions: Optional[List[str]]=None, skip_hidden: bool=True) -> Dict:
    """
    ...
    tree = {'name': root_path.name, 'type': 'dir', 'path': str(root_path), 'lines': 0, 'children': {}}
    for item in root_path.iterdir():
        ...
        if item.is_dir():
            ...
            tree['children'][item.name] = {'type': 'file', 'name': item.name, 'path': str(item), 'lines': lines}
            tree['lines'] += lines
    return tree
...
def tree_to_dataframe(tree: Dict) -> pd.DataFrame:
    ...
    def dfs(node, parent_path=None):
        rows.append({'path': node['path'], 'name': node['name'], 'type': node['type'], 'lines': node['lines']})
        if node['type'] == 'dir':
...
def print_tree(tree: Dict, indent: int=0) -> None:
    ...
    prefix = '  ' * indent
    if tree['type'] == 'dir':
        logger.info(f"{prefix}📁 {tree['name']} (total lines: {tree['lines']})")
        for child in tree['children'].values():
            print_tree(child, indent + 1)
    else:
        logger.info(f"{prefix}📄 {tree['name']} - {tree['lines']} lines")
if __name__ == '__main__':
    ...
    total_lines = df[df['type'] == 'dir'].iloc[0]['lines']
