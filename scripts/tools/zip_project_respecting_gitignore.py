...
def run_git_ls_files(project_root: Path, include_untracked: bool) -> list[Path]:
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            "Failed to read project files via git ls-files. "
            "Make sure the target folder is a Git repository and Git is installed.\n"
            f"Git error: {stderr}"
        )
    ...
    with zipfile.ZipFile(
        ...
        for relative_path in files_to_zip:
            ...
            if not absolute_path.exists():
                print(f"Warning: skipped missing file: {relative_path}", file=sys.stderr)
                continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create a ZIP archive of a Git project while respecting .gitignore, "
            "nested .gitignore files, .git/info/exclude, and global git excludes."
        )
    )
    ...
    parser.add_argument(
        ...
        "--output",
        default=None,
        help="Output ZIP path. Default: ../<project_name>_<timestamp>.zip",
    )

    parser.add_argument(
        "--tracked-only",
        action="store_true",
        help="Include only tracked Git files. By default, tracked + untracked non-ignored files are included.",
    )

    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print files that would be archived without creating ZIP.",
    )
    ...
    parser.add_argument(
        ...
        choices=range(0, 10),
        metavar="0-9",
        help="ZIP compression level. 0 = no compression, 9 = maximum. Default: 6.",
    )
