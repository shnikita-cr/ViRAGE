...
def _record_from_uie_file(path: Path, project_root: Path) -> dict[str, Any] | None:
    ...
    return {
        "record_type": "chartuie_visual_requirement_seed",
        "source_dataset": "ChartSquared/ChartUIE-8K",
        "source_id": path.stem,
        "raw_record_path": rel,
        "retrieval_text": query,
        "user_query": query,
        "visual_requirement_seed": {
            "must_be_visible": [],
            "critical_failures": [
                "Required information is not visible in the static chart image.",
                "Requested grouping or highlighting is only available through tooltip or hidden interaction.",
            ],
            "yes_no_questions": [
                "Does the chart image visibly satisfy the user request?",
                "Are the requested variables readable in axes, labels, legend, or panels?",
            ],
        },
