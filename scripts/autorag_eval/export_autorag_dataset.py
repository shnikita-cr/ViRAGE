...
def _record_contents(record: dict[str, Any]) -> str:
    ...
    parts: list[str] = [f'Title: {title}', f'Source: {_source_id(record)} {_source_name(record)}', f'Record type: {_record_type(record)}', f"Chart family: {metadata.get('chart_family') or ''}", f"Task: {metadata.get('task') or ''}", f"Applies when: {'; '.join(applies_when)}", f"Guidance: {'; '.join(guidance)}", f"Avoid: {'; '.join(avoid)}", str(text)]
...
def _corpus_rows(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        rows.append({'doc_id': doc_id, 'contents': _record_contents(record), 'metadata': metadata})
    return rows
...
def _resolve_retrieval_gt(*, query: dict[str, Any], records: list[dict[str, Any]], max_gt_per_query: int) -> list[str]:
...
def _qa_rows(*, queries: list[dict[str, Any]], records: list[dict[str, Any]], max_gt_per_query: int, allow_empty_gt: bool) -> tuple[list[dict[str, Any]], list[str]]:
    rows: list[dict[str, Any]] = []
    empty_qids: list[str] = []
    for index, query in enumerate(queries, start=1):
        ...
        if not retrieval_gt:
            ...
            if not allow_empty_gt:
                continue
        rows.append({'qid': qid, 'query': text, 'generation_gt': str(query.get('generation_gt') or text), 'retrieval_gt': retrieval_gt, 'metadata': {'expected_topics': _string_list(query.get('expected_topics')), 'query_type': query.get('query_type'), 'source_query': qid}})
    return (rows, empty_qids)

def export_autorag_dataset(*, corpus_path: Path, queries_path: Path, output_dir: Path, max_gt_per_query: int=20, allow_empty_gt: bool=False) -> dict[str, Any]:
    ...
    qa_rows, empty_qids = _qa_rows(queries=queries, records=records, max_gt_per_query=max_gt_per_query, allow_empty_gt=allow_empty_gt)
    if empty_qids and (not allow_empty_gt):
        raise AutoRAGDatasetExportError(f"Some queries have empty retrieval_gt after resolving labels. Fix rag_corpus/autorag/qa/retrieval_queries.jsonl. Empty qids: {', '.join(empty_qids[:20])}")
    if not qa_rows:
    ...
    report = {'corpus_input': str(corpus_path), 'queries_input': str(queries_path), 'output_dir': str(output_dir), 'corpus': {'records': len(corpus_rows), 'path': str(corpus_output), 'by_source': source_counts}, 'qa': {'records': len(qa_rows), 'path': str(qa_output), 'empty_qids': empty_qids}, 'max_gt_per_query': max_gt_per_query, 'allow_empty_gt': allow_empty_gt}
    write_json(output_dir / 'dataset_manifest.json', report)
    write_text(output_dir / 'dataset_manifest.md', '\n'.join(['# ViRAGE AutoRAG dataset export', '', f'Corpus input: `{corpus_path}`', f'Queries input: `{queries_path}`', f'Corpus records: **{len(corpus_rows)}**', f'QA records: **{len(qa_rows)}**', f'Corpus parquet: `{corpus_output}`', f'QA parquet: `{qa_output}`', '']))
    return report
...
def main() -> None:
    args = parse_args()
    report = export_autorag_dataset(corpus_path=ROOT / args.corpus, queries_path=ROOT / args.queries, output_dir=ROOT / args.output_dir, max_gt_per_query=args.max_gt_per_query, allow_empty_gt=args.allow_empty_gt)
