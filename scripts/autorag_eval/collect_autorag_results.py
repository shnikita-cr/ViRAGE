...
def collect_autorag_results(*, project_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if not summary_paths:
        raise FileNotFoundError(
            f"No AutoRAG summary.csv files found under {project_dir}. Run autorag evaluate first."
        )
...
def _markdown(*, summary_paths: list[Path], combined: pd.DataFrame, csv_path: Path) -> str:
    lines = [
        "# ViRAGE AutoRAG evaluation report",
        "",
        "This report only aggregates AutoRAG `summary.csv` files. It does not recompute retrieval metrics.",
        "",
...
def main() -> None:
    ...
    print(json.dumps({"rows": len(report["rows"]), "output_csv": report["output_csv"]}, ensure_ascii=False, indent=2), flush=True)
