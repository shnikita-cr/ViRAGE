...
def load(root: Path, *, refresh: bool = False, timeout_seconds: float = 60.0) -> dict[str, object]:
    return download_html_pages(
        ...
        LoaderConfig(
            ...
            seed_urls=("https://analysisfunction.civilservice.gov.uk/policy-store/data-visualisation-colours-in-charts/",),
            allowed_hosts=("analysisfunction.civilservice.gov.uk",),
            allowed_path_prefixes=("/policy-store/data-visualisation-colours-in-charts/",),
...
def main() -> None:
    run_loader_cli(load, "Download UK Analysis Function colour guidance into rag_corpus/raw_external_rules.")
