...
from scripts.rag_corpus.exporters.registry.source_registry import QUALITY_CORPUS_BY_ID
from scripts.rag_corpus.loading.core.fetch import decode_payload, fetch_bytes, looks_like_failed_download
...
from scripts.rag_corpus.loading.core.urls import matches_allowed_scope, normalise_candidate_url, relative_text_path_for_url
...
def _validate_cached_txt(path: Path, *, min_text_chars: int) -> int:
    ...
    if size < min_text_chars:
        raise SourceDownloadError(f'Cached cleaned text is too small: {path}: {size} bytes, expected at least {min_text_chars}. Run with --refresh after fixing the source URL.')
    return size
...
def _extract_marker_guided_text_from_html(html_text: str, *, page_url: str, config: LoaderConfig) -> str:
    ...
    target_tags = ('h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'li', 'dt', 'dd', 'figcaption', 'caption', 'blockquote', 'td', 'th', 'span', 'div')
    for element in soup.find_all(target_tags):
        ...
        if any((noise in lowered for noise in ('manage cookies', 'privacy policy', 'terms & conditions'))):
            continue
        seen.add(lowered)
        if element.name and re.fullmatch('h[1-6]', element.name):
    ...
    if markers:
        ...
        for script in soup.find_all('script'):
            ...
            if not any((marker in normalised_script for marker in marker_terms)):
                continue
            cleaned_script = re.sub('[{}\\[\\]\\"\'<>]', ' ', script_text)
...
def _fetch_clean_page(url: str, config: LoaderConfig, *, timeout_seconds: float) -> tuple[str, str, int, str]:
    ...
    if len(payload) < config.min_bytes:
        raise SourceDownloadError(f'Downloaded source is too small: {url}: {len(payload)} bytes, expected at least {config.min_bytes}')
    ...
    if failed_like and (not has_required_marker):
        raise SourceDownloadError(f'Downloaded page looks like access/error page: {url}; html_chars={len(html_text)}; content_type={content_type!r}.')
    if config.required_text_markers and (not has_required_marker):
        raise SourceDownloadError(f"Downloaded page does not contain required guidance markers: {url}. Expected one of: {', '.join(config.required_text_markers)}")
    if len(cleaned.strip()) < config.min_text_chars:
        raise SourceDownloadError(f'Extracted cleaned text is too small: {url}: {len(cleaned.strip())} chars, expected at least {config.min_text_chars}. The page may not contain useful guidance.')
...
def download_html_pages(root: Path, config: LoaderConfig, *, refresh: bool, timeout_seconds: float) -> dict[str, object]:
    ...
    while queue and len(downloaded_urls) < config.max_pages:
        ...
        try:
            ...
            if url in seed_urls or not config.skip_discovered_errors:
                raise
            items.append({'status': 'skipped_discovered_error', 'url': url, 'error': str(exc)})
        ...
        items.append({'status': 'downloaded', 'url': url, 'path': str(output_path), 'bytes': output_path.stat().st_size, 'raw_bytes': raw_bytes, 'content_type': content_type})
        downloaded_urls.add(url)
    ...
    if len(successful_items) < config.min_pages:
        raise SourceDownloadError(f'Only {len(successful_items)} usable pages downloaded for source {config.source_id}; expected at least {config.min_pages}.')
    manifest = {'source_id': config.source_id, 'status': 'ok', 'items': items, 'downloaded_pages': len(successful_items), 'skipped_pages': len(items) - len(successful_items), 'target_dir': str(target_dir), 'saved_format': 'clean_txt'}
    write_json(target_dir / 'download_manifest.json', manifest)
    return manifest

def download_text_file(root: Path, *, source_id: str, url: str, relative_path: str, refresh: bool, timeout_seconds: float, min_bytes: int=500) -> dict[str, object]:
