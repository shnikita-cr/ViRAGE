...
def build_report(path: Path) -> dict[str, Any]:
    ...
    return {'status': 'WARN' if warnings else 'PASS', 'input': path.as_posix(), 'total': len(records), 'by_source': dict(source_counts.most_common()), 'by_source_kind': dict(kind_counts.most_common()), 'text_chars': _length_stats([len(text) for text in texts]), 'top_repeated_text': [{'count': count, 'text': text[:200]} for text, count in repeated_text.most_common(30) if count > 1], 'shortest_records': _shortest_records(records), 'noise_examples': noise_examples, 'warnings': warnings}

def render_markdown(report: dict[str, Any]) -> str:
    lines = ['# Runtime RAG corpus quality report', '', f"Status: **{report['status']}**", f"Input: `{report['input']}`", f"Total chunks: `{report['total']}`", '', '## Warnings']
...
def _warnings(*, records: list[dict[str, Any]], texts: list[str], source_counts: Counter[str], noise_examples: list[dict[str, Any]]) -> list[str]:
    warnings: list[str] = []
