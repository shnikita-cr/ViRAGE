...
def extract_manual_rules(input_dir: Path) -> list[SourceRecord]:
    ...
    for path in sorted(input_dir.rglob('*')):
        ...
        for idx, item in enumerate(_items(payload)):
            ...
            text_parts = [title, compact_text(item.get('description')), compact_text(item.get('applies_when')), compact_text(item.get('guidance')), compact_text(item.get('avoid')), compact_text(item.get('retrieval_text'))]
            ...
            records.append(SourceRecord(record_id=record_id, source_dataset='manual_rules', source_path=str(path), source_type='manual_rule_seed', title=title, text=text, task=item.get('task'), chart_family=item.get('chart_family'), metadata={'preferred_record_type': item.get('record_type')}, raw=item))
    return records
