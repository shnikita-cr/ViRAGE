    ...
    for path in sorted(input_dir.rglob("*.jsonl")):
        for idx, item in enumerate(_read_jsonl(path)):
            ...
            records.append(SourceRecord(
                record_id=record_id,
                source_dataset="virage_feedback",
                source_path=str(path),
                source_type="visual_or_user_feedback",
                title=compact_text(item.get("title") or item.get("query") or "ViRAGE feedback"),
                text=text,
                task=item.get("analysis_task") or item.get("task"),
                chart_family=item.get("chart_family") or item.get("chart_type"),
                metadata={"feedback_kind": item.get("feedback_kind") or item.get("source")},
                raw=item,
            ))
    return records
