...
TEXT_KEYS = ('question', 'query', 'instruction', 'request', 'prompt', 'initial_prompt', 'feedback', 'critique', 'comment', 'answer', 'answers', 'questions', 'task', 'purpose', 'audience', 'chart_type', 'chart', 'caption', 'description', 'criteria', 'evaluation', 'data_attributes', 'messages', 'content', 'text')
...
def _is_prompt_file(path: Path, input_dir: Path) -> bool:
    ...
    return 'prompts' in relative_parts or stem.startswith('af_') or 'prompt' in stem or (stem in {'criteria', 'feedback', 'eval', 'evaluation'})
...
def extract_chartsquared(input_dir: Path, *, mode: ChartSquaredMode='sample', sample_limit: int | None=300, show_progress: bool=True) -> list[SourceRecord]:
    ...
    for path in paths:
        ...
        for idx, raw_item in enumerate(loaded_records):
            ...
            records.append(SourceRecord(record_id=record_id, source_dataset='chartsquared', source_path=str(path), source_type=source_kind, title=compact_text(title_value), text=text, task=item.get('task') if isinstance(item.get('task'), str) else None, chart_family=item.get('chart_type') or item.get('chart') if isinstance(item.get('chart_type') or item.get('chart'), str) else None, metadata={'relative_source_path': str(path.relative_to(input_dir)) if path.is_relative_to(input_dir) else str(path), 'chartsquared_mode': mode, 'is_prompt_file': _is_prompt_file(path, input_dir), 'source_weight': 0.75 if not _is_prompt_file(path, input_dir) else 1.0, 'requires_manual_review_before_runtime': mode == 'full'}, raw=item))
