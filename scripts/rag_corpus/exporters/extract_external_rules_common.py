...
from scripts.rag_corpus.exporters.extractor_filters import is_relevant_visualization_source
from scripts.rag_corpus.exporters.extractor_html import extract_html_sections
from scripts.rag_corpus.exporters.extractor_structured import (
    chunk_text,
    flatten_json,
    load_json_like_records,
    split_markdown_sections,
)
from scripts.rag_corpus.exporters.extractor_text import (
    CODE_SUFFIXES,
    DATA_SUFFIXES,
...
def extract_markdown_like(
    ...
    *,
    ...
    for path in files:
        ...
        records.extend(_records_from_sections(
            input_dir=input_dir,
            path=path,
            sections=sections,
            source_dataset=source_dataset,
...
def extract_json_like(
    ...
    *,
    ...
    for path in files:
        ...
        records.extend(_records_from_json_items(
            input_dir=input_dir,
            path=path,
            items=loaded,
            source_dataset=source_dataset,
    ...
    *,
    ...
    for idx, payload in enumerate(items):
        ...
        records.append(make_source_record(
            ...
            source_dataset=source_dataset,
            source_type=source_type,
            title=f"{title} #{idx + 1}" if len(items) > 1 else title,
            text=text,
            preferred_record_type=preferred_record_type,
            record_prefix=record_prefix,
            raw=payload,
