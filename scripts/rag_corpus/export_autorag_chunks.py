...
_KEYWORD_PATTERNS: list[tuple[str, str]] = [
    (r"\b(overplot|overlap|scatter|density|jitter|transparent|alpha)\b", "dense scatter plots and overlapping points"),
    (r"\b(color|colour|palette|hue|sequential|diverging|qualitative|rainbow)\b", "chart color, palette, and color-scale choice"),
    (r"\b(label|legend|title|caption|axis|axes|unit)\b", "chart labels, titles, axes, legends, and units"),
    (r"\b(histogram|density|distribution|bin|outlier|boxplot|violin)\b", "distribution charts, bins, density, and outliers"),
    (r"\b(bar|category|categories|categorical|sort|order|rank|ranking)\b", "categorical comparison, ranking, sorting, and many categories"),
    (r"\b(line|time|trend|series|spaghetti)\b", "time-series charts, trends, and too many lines"),
    (r"\b(accessib|contrast|screen reader|alt text|colour blind|color blind)\b", "visualization accessibility and non-color encodings"),
    (r"\b(area|baseline|zero|proportional|scale|log)\b", "axes, baseline, scales, and proportional visual size"),
]


def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    if not path.exists():
        raise FileNotFoundError(f"Guidance chunks file not found: {path}. Run scripts/rag_corpus/export_guidance_chunks.py first.")
...
def _contents(row: dict[str, Any]) -> str:
    ...
    parts = [
        ...
        f"Source: {row.get('source_name') or row.get('source_id') or ''}",
        f"Source kind: {row.get('source_kind') or ''}",
        f"Source path: {row.get('source_path') or ''}",
        f"Chunk index: {metadata.get('chunk_index') or ''}",
        "Guidance source text:",
        str(row.get("text") or ""),
    ]
...
def _corpus_rows(chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in chunks:
        ...
        metadata.update({
            "record_type": str(row.get("source_kind") or "web_guidance"),
            "source_kind": str(row.get("source_kind") or "web_guidance"),
...
def _qa_rows(chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in chunks:
        ...
        rows.append({
            "qid": f"qa__chunk__{chunk_id}",
            "query": _qa_query(row),
            "generation_gt": text[:1200],
            "retrieval_gt": [chunk_id],
            "metadata": {
...
def export_autorag_chunks(
    *,
    ...
    report = {
        ...
        "corpus": {"records": len(corpus_rows), "output": str(corpus_path)},
        "qa": {"questions": len(qa_rows), "output": str(qa_path)},
        "config": str(config_path),
        "split": split_report,
    }
    write_json(output_root / "export_report.json", report)
    write_text(
        output_root / "export_report.md",
        "\n".join([
            "# VisRAG chunks AutoRAG export",
            "",
            f"Input: `{input_path}`",
            f"Corpus records: **{len(corpus_rows)}**",
            f"QA questions: **{len(qa_rows)}**",
