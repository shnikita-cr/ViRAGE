...
SKIP_FILE_RE = re.compile('download_manifest|bibliography|references|image-file-formats|choosing-visualization-software', re.I)
...
def chunk_text(text: str, *, min_chars: int=DEFAULT_MIN_CHARS, max_chars: int=DEFAULT_MAX_CHARS, overlap_chars: int=DEFAULT_OVERLAP_CHARS) -> list[str]:
...
def build_chunk(path: Path, raw_root: Path, text: str, index: int, *, original_text_chars: int, max_chars: int, overlap_chars: int) -> dict[str, object]:
    ...
    return {'chunk_id': f'chunk_{stable}', 'source_id': source_id, 'source_name': source_id.replace('_', ' '), 'source_kind': source_kind_for(source_id), 'title': title, 'text': text, 'source_path': rel, 'url': None, 'metadata': {'chunk_index': index, 'source_file': rel, 'text_sha1': hashlib.sha1(text.encode('utf-8')).hexdigest(), 'chunk_char_length': len(text), 'original_text_chars': original_text_chars, 'chunk_max_chars': max_chars, 'chunk_overlap_chars': overlap_chars}}
...
def _runtime_length_report(records: list[dict[str, object]], *, max_chars: int) -> dict[str, object]:
    ...
    longest = sorted([{'chunk_id': str(record.get('chunk_id') or ''), 'source_id': str(record.get('source_id') or ''), 'source_path': str(record.get('source_path') or ''), 'title': str(record.get('title') or ''), 'char_length': len(str(record.get('text') or ''))} for record in records], key=lambda item: item['char_length'], reverse=True)[:20]
    return {'max_allowed_chars': max_chars, 'total_chunks': len(records), 'min_chunk_chars': min(lengths) if lengths else 0, 'max_chunk_chars': max(lengths) if lengths else 0, 'avg_chunk_chars': round(sum(lengths) / len(lengths), 2) if lengths else 0, 'oversized_chunks': len(oversized), 'by_source': by_source, 'longest_chunks': longest}

def export_chunks(raw_root: Path, output: Path, sources: set[str] | None, *, min_chars: int=DEFAULT_MIN_CHARS, max_chars: int=DEFAULT_MAX_CHARS, overlap_chars: int=DEFAULT_OVERLAP_CHARS) -> dict[str, object]:
    _validate_chunking_params(min_chars=min_chars, max_chars=max_chars, overlap_chars=overlap_chars)
    ...
    for path in iter_source_files(raw_root, sources):
        ...
        for index, chunk in enumerate(chunks, start=1):
            records.append(build_chunk(path, raw_root, chunk, index, original_text_chars=len(text), max_chars=max_chars, overlap_chars=overlap_chars))
    if not records:
    ...
    return {'output': output.as_posix(), 'source_files': source_file_count, 'by_source': by_source, 'total_chunks': sum(by_source.values()), 'chunking': {'min_chars': min_chars, 'max_chars': max_chars, 'overlap_chars': overlap_chars}, 'length_report': length_report}
...
def main() -> None:
    ...
    report = export_chunks(ROOT / args.raw_root, ROOT / args.output, set(args.sources or []) or None, min_chars=args.min_chars, max_chars=args.max_chars, overlap_chars=args.overlap_chars)
