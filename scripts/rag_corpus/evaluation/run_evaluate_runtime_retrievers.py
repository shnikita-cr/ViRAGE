...
def _load_corpus(path: Path) -> list[VisRAGRuleDocument]:
    ...
    for row in df.to_dict(orient='records'):
        ...
        documents.append(VisRAGRuleDocument(doc_id=str(row['doc_id']), record_type=record_type, title=str(metadata.get('title') or row['doc_id']), retrieval_text=str(row.get('contents') or ''), prompt_text=str(row.get('contents') or ''), metadata=metadata))
    return documents
...
def _load_questions(path: Path) -> list[RetrievalQuestion]:
    ...
    for row in df.to_dict(orient='records'):
        ...
        if not isinstance(retrieval_gt, list):
            retrieval_gt = []
        questions.append(RetrievalQuestion(qid=str(row['qid']), query=str(row['query']), retrieval_gt=[str(item) for item in retrieval_gt], record_type=str(metadata.get('record_type') or '') or None))
    return questions
...
def _query_analysis(query: str) -> QueryRequestAnalysisResult:
    return QueryRequestAnalysisResult(normalized_query=query, analysis_task='retrieval_evaluation', confidence=1.0)

def _evaluate_backend(*, backend: str, documents: list[VisRAGRuleDocument], questions: list[RetrievalQuestion], top_k_values: list[int], embedding_provider: str | None, embedding_model: str | None, embedding_base_url: str | None) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    options = RuleRetrieverOptions(backend=backend, embedding_provider=embedding_provider, embedding_model=embedding_model, embedding_base_url=embedding_base_url)
    ...
    for question in questions:
        ...
        ranked = retriever.rank(candidate_documents, query=question.query, query_analysis=_query_analysis(question.query), corpus_key=f"autorag_eval:{backend}:{question.record_type or 'all'}")
        ranked_ids = [item.doc_id for item in ranked[:max_k]]
        row: dict[str, Any] = {'backend': backend, 'qid': question.qid, 'record_type': question.record_type or '', 'gt': json.dumps(question.retrieval_gt, ensure_ascii=False), 'top_docs': json.dumps(ranked_ids, ensure_ascii=False)}
...
def _best_rows(metric_rows: list[dict[str, Any]], select_by: str) -> dict[str, dict[str, Any]]:
    ...
    for row in metric_rows:
        ...
        candidate_key = (float(row.get(select_by, 0.0)), float(row.get('mrr', 0.0)), float(row.get('hit', 0.0)), -int(row.get('top_k', 0)))
        if current is None:
            best[record_type] = row
            continue
        current_key = (float(current.get(select_by, 0.0)), float(current.get('mrr', 0.0)), float(current.get('hit', 0.0)), -int(current.get('top_k', 0)))
...
def _runtime_config_toml(best: dict[str, dict[str, Any]]) -> str:
    ...
    lines = ['[settings]', f'visrag_retrieval_backend = "{retrieval_backend}"', f'visrag_top_k_chunks = {top_k}', 'visrag_embedding_provider = "ollama"', 'visrag_embedding_model = "nomic-embed-text:latest"', 'visrag_embedding_base_url = "http://localhost:11434"', 'visrag_chroma_persist_dir = "./resources/chroma/virage_guidance_chunks_nomic_embed_text_latest"', 'visrag_chroma_collection_name = "virage_guidance_chunks_nomic_embed_text_latest"']
    return '\n'.join(lines) + '\n'

def _markdown_summary(metric_rows: list[dict[str, Any]], best: dict[str, dict[str, Any]], select_by: str) -> str:
    lines = ['# Runtime retrieval evaluation', '', f'Selection metric: `{select_by}`', '', '## Recommended runtime settings', '', '```toml', _runtime_config_toml(best).strip(), '```', '', '## Best rows', '', '| record_type | backend | top_k | hit | recall | precision | mrr | ndcg | cases |', '|---|---:|---:|---:|---:|---:|---:|---:|---:|']
    for record_type, row in sorted(best.items()):
        lines.append(f"| {record_type} | {row.get('backend')} | {row.get('top_k')} | {row.get('hit')} | {row.get('recall')} | {row.get('precision')} | {row.get('mrr')} | {row.get('ndcg')} | {row.get('cases')} |")
...
def main() -> None:
    ...
    for backend in args.backends:
        case_rows, metric_rows = _evaluate_backend(backend=backend, documents=documents, questions=questions, top_k_values=top_k_values, embedding_provider=args.embedding_provider, embedding_model=args.embedding_model, embedding_base_url=args.embedding_base_url)
    ...
    logger.info(json.dumps({'cases': len(questions), 'documents': len(documents), 'output_dir': str(output_dir), 'best': best.get('__all__')}, ensure_ascii=False, indent=2))
