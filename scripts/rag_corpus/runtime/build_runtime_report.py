...
def build_report(input_path: Path) -> dict[str, Any]:
    ...
    return {'documents': len(docs), 'by_record_type': dict(sorted(by_type.items())), 'avg_prompt_text_chars': round(sum(prompt_lengths) / len(prompt_lengths), 2) if prompt_lengths else 0, 'avg_retrieval_text_chars': round(sum(retrieval_lengths) / len(retrieval_lengths), 2) if retrieval_lengths else 0, 'input': str(input_path)}
