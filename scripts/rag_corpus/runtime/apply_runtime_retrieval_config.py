...
ALLOWED_KEYS = {'visrag_retrieval_backend', 'visrag_top_k_chunks', 'visrag_hybrid_method', 'visrag_hybrid_weight', 'visrag_hybrid_rrf_k', 'visrag_candidate_pool_size', 'visrag_embedding_provider', 'visrag_embedding_model', 'visrag_embedding_base_url', 'visrag_chroma_persist_dir', 'visrag_chroma_collection_name'}
...
def _replace_setting(text: str, key: str, value: Any) -> str:
    pattern = re.compile(f'^(?P<prefix>\\s*{re.escape(key)}\\s*=\\s*)(?P<value>.*)$', re.MULTILINE)
    replacement = f'\\g<prefix>{_toml_value(value)}'
    if pattern.search(text):
        return pattern.sub(replacement, text, count=1)
    settings_match = re.search('^\\[settings\\]\\s*$', text, flags=re.MULTILINE)
