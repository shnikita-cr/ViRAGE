...
from src.visrag_core.indexing.chroma_index import chunk_embedding_text, normalize_chroma_metadata, validate_chunks_for_index, write_chroma_manifest
...
def build_runtime_chroma_index(*, chunks_path: Path, persist_dir: Path, collection_name: str, embedding_provider: str, embedding_model: str, embedding_base_url: str | None, batch_size: int, max_input_chars: int, recreate: bool) -> dict[str, Any]:
    ...
    embedder = build_embedding_model(provider=embedding_provider, model=embedding_model, base_url=embedding_base_url)
    ...
    for batch in batches:
        ...
        for chunk, vector in zip(batch, vectors, strict=True):
            ...
            if embedding_dimension is None:
                ...
                raise RuntimeError(f'Embedding dimension mismatch for chunk_id={chunk.chunk_id!r}: expected {embedding_dimension}, got {len(vector)}.')
        metadatas = [normalize_chroma_metadata({**(chunk.metadata or {}), 'chunk_id': chunk.chunk_id, 'source_id': chunk.source_id, 'source_name': chunk.source_name, 'source_kind': chunk.source_kind, 'title': chunk.title, 'source_path': chunk.source_path or '', 'url': chunk.url or ''}) for chunk in batch]
    ...
    manifest = write_chroma_manifest(persist_dir=persist_dir, chunks_path=chunks_path, collection_name=collection_name, embedding_provider=embedding_provider, embedding_model=embedding_model, embedding_dimension=embedding_dimension, chunk_count=len(chunks))
...
def main() -> None:
    args = parse_args()
    report = build_runtime_chroma_index(chunks_path=Path(args.chunks), persist_dir=Path(args.persist_dir), collection_name=str(args.collection), embedding_provider=str(args.embedding_provider), embedding_model=str(args.embedding_model), embedding_base_url=str(args.embedding_base_url or '') or None, batch_size=int(args.batch_size), max_input_chars=int(args.max_input_chars), recreate=bool(args.recreate))
