...
def _to_chunk(record: NormalizedFeedbackRecord) -> dict[str, Any]:
    ...
    return {
        ...
        "source_id": source_id,
        "source_name": "Manual user feedback" if source_id == "manual_feedback" else "VLM chart feedback",
        "source_kind": record.source_kind,
        "title": f"{record.feedback_type}: {record.chart_family or record.task_type or 'chart feedback'}",
        "text": text,
        "source_path": None,
        "url": None,
        "metadata": {
            ...
            "fields_used": record.fields_used,
            "priority": record.priority,
            "approved_for_rag": record.approved_for_rag,
            "approved_for_retrieval": record.approved_for_rag,
...
def export_feedback_chunks(
    *,
    raw_path: Path = DEFAULT_RAW,
    normalized_path: Path = DEFAULT_NORMALIZED,
    output_chunks_path: Path = DEFAULT_CHUNKS,
    mode: str = "rules",
    ...
    report = {
        ...
        "output_chunks_path": output_chunks_path.as_posix(),
        "mode": mode,
        "raw_records": len(raw_records),
        "normalized_records": len(normalized),
        "approved_records": len(approved),
        "exported_chunks": len(chunks),
        "base_corpus": base_corpus.as_posix() if base_corpus else None,
        "merged_output": merged_output.as_posix() if merged_output else None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = export_feedback_chunks(
        ...
        output_chunks_path=Path(args.output),
        mode=args.mode,
        runtime=runtime,
        approve_all=bool(args.approve_all),
        normalized_only=bool(args.normalized_only),
        base_corpus=Path(args.base_corpus) if args.base_corpus else None,
