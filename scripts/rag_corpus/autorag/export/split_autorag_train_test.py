...
def split_autorag_data(*, corpus_path: Path, qa_path: Path, output_root: Path, train_ratio: float, seed: int) -> dict[str, Any]:
    ...
    for row in qa_df.to_dict(orient='records'):
        ...
        assignments.append({'qid': qid, 'split': split, 'retrieval_gt': json.dumps(sorted(gt), ensure_ascii=False)})
    ...
    if train_qa.empty or test_qa.empty:
        raise ValueError(f'Train/test split produced an empty QA subset. Check qa.parquet retrieval_gt values and split ratio. train_questions={len(train_qa)}, test_questions={len(test_qa)}, dropped_questions={len(qa_rows_dropped)}')
    ...
    for record_type, group in corpus_with_meta.groupby('record_type', sort=True):
        by_type[record_type] = {'train_docs': int((group['_split'] == 'train').sum()), 'test_docs': int((group['_split'] == 'test').sum())}
    report: dict[str, Any] = {'train_ratio': train_ratio, 'seed': seed, 'corpus': {'total_docs': int(len(corpus_df)), 'train_docs': int(len(train_corpus)), 'test_docs': int(len(test_corpus)), 'by_record_type': by_type}, 'qa': {'total_questions': int(len(qa_df)), 'train_questions': int(len(train_qa)), 'test_questions': int(len(test_qa)), 'dropped_questions': int(len(qa_rows_dropped))}, 'paths': {'train_corpus': str(train_dir / 'corpus.parquet'), 'train_qa': str(train_dir / 'qa.parquet'), 'test_corpus': str(test_dir / 'corpus.parquet'), 'test_qa': str(test_dir / 'qa.parquet'), 'assignments': str(output_root / 'split_assignments.csv')}}
...
def _report_md(report: dict[str, Any]) -> str:
    lines = ['# AutoRAG train/test split', '', f"Train ratio: **{report['train_ratio']}**", f"Seed: **{report['seed']}**", '', '## Corpus', '', f"Total docs: **{report['corpus']['total_docs']}**", f"Train docs: **{report['corpus']['train_docs']}**", f"Test docs: **{report['corpus']['test_docs']}**", '', '| Record type | Train docs | Test docs |', '|---|---:|---:|']
    ...
    lines.extend(['', '## QA', '', f"Total questions: **{report['qa']['total_questions']}**", f"Train questions: **{report['qa']['train_questions']}**", f"Test questions: **{report['qa']['test_questions']}**", f"Dropped questions: **{report['qa']['dropped_questions']}**", '', '## Paths', ''])
...
def main() -> None:
    ...
    root = project_root()
    report = split_autorag_data(corpus_path=root / args.corpus, qa_path=root / args.qa, output_root=root / args.output_root, train_ratio=args.train_ratio, seed=args.seed)
