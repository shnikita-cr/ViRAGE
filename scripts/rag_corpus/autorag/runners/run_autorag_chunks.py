...
def _validate_parquet(path: Path, *, required_columns: set[str], label: str) -> dict[str, Any]:
    ...
    return {'path': str(path), 'rows': int(len(frame)), 'columns': list(frame.columns), 'null_counts': null_counts}

def _preflight_autorag_cli(autorag_command: str, *, cwd: Path, env: dict[str, str]) -> None:
    try:
        result = subprocess.run([autorag_command, '--help'], cwd=cwd, env=env, check=False, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    except FileNotFoundError as exc:
        raise AutoRAGRunnerError('AutoRAG CLI command was not found. Install API AutoRAG without GPU extras:\npip install AutoRAG fastapi gradio chromadb pyarrow pandas scikit-learn llama-index-embeddings-ollama') from exc
    output = result.stdout or ''
    if result.returncode != 0:
        raise AutoRAGRunnerError(f'AutoRAG CLI is installed but cannot start. This project uses the API AutoRAG package, not AutoRAG[gpu] and not vLLM.\nInstall/repair the API dependencies, for example:\npip install --upgrade AutoRAG fastapi gradio chromadb pyarrow pandas scikit-learn llama-index-embeddings-ollama\n\nAutoRAG output:\n{output}')
...
def run_validate(*, autorag_command: str, config: Path, qa_path: Path, corpus_path: Path) -> dict[str, Any]:
    ...
    _run([autorag_command, 'validate', '--config', str(config), '--qa_data_path', str(qa_path), '--corpus_data_path', str(corpus_path)], cwd=ROOT, env=env)
    return {'stage': 'validate', 'status': 'ok', 'config': str(config), 'qa': qa_report, 'corpus': corpus_report}

def run_evaluate(*, autorag_command: str, config: Path, qa_path: Path, corpus_path: Path, project_dir: Path, skip_validation: bool, clean_project_dir: bool) -> dict[str, Any]:
    _require_file(config, 'AutoRAG YAML config')
    qa_report = _validate_parquet(qa_path, required_columns={'qid', 'query', 'retrieval_gt'}, label='AutoRAG QA parquet')
    corpus_report = _validate_parquet(corpus_path, required_columns={'doc_id', 'contents'}, label='AutoRAG corpus parquet')
    ...
    env = _prepare_env()
    _preflight_autorag_cli(autorag_command, cwd=ROOT, env=env)
    command = [autorag_command, 'evaluate', '--config', str(config), '--qa_data_path', str(qa_path), '--corpus_data_path', str(corpus_path), '--project_dir', str(project_dir)]
    if skip_validation:
        command.append('--skip_validation')
    _run(command, cwd=ROOT, env=env)
    return {'stage': 'evaluate', 'status': 'ok', 'config': str(config), 'project_dir': str(project_dir), 'skip_validation': skip_validation, 'qa': qa_report, 'corpus': corpus_report}

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run AutoRAG for VisRAG chunks through the AutoRAG API CLI and YAML config. This runner does not require AutoRAG[gpu], vLLM, or local model extras.')
...
def main() -> None:
    ...
    if args.mode in {'validate', 'both'}:
        reports.append(run_validate(autorag_command=args.autorag_command, config=config, qa_path=qa_path, corpus_path=corpus_path))
    if args.mode in {'evaluate', 'both'}:
        reports.append(run_evaluate(autorag_command=args.autorag_command, config=config, qa_path=qa_path, corpus_path=corpus_path, project_dir=project_dir, skip_validation=args.skip_validation, clean_project_dir=args.clean_project_dir))
    logger.info(json.dumps({'reports': reports}, ensure_ascii=False, indent=2))
