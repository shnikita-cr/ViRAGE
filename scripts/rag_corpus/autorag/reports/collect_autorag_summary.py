...
def collect_summary(runs_root: Path) -> list[dict[str, Any]]:
    ...
    for csv_path in sorted(runs_root.rglob('*.csv')):
        ...
        if not metric_cols:
            rows.append({'run': _run_name(runs_root, csv_path), 'file': str(csv_path), 'rows': int(len(df)), 'note': 'no numeric retrieval metric columns detected'})
            continue
        for metric in metric_cols:
            best_idx = df[metric].astype(float).idxmax()
            best_row = df.loc[best_idx].to_dict()
            row: dict[str, Any] = {'run': _run_name(runs_root, csv_path), 'file': str(csv_path), 'rows': int(len(df)), 'selected_metric': metric, 'selected_metric_value': float(best_row.get(metric))}
...
def main() -> None:
    ...
    logger.info(json.dumps({'rows': len(rows), 'output_dir': str(output_dir)}, ensure_ascii=False, indent=2))
