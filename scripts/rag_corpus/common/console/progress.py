...
class StageProgress:
    ...
    def render(self, *, extra: str | None=None) -> None:
        ...
        message = f'[{self.stage}] {self.done}/{total_text}{percent} | errors={self.errors} | elapsed={_format_seconds(elapsed)} | eta={_format_seconds(eta)}'
        if extra:
            message += f' | {extra}'
        padding = ' ' * max(0, self._last_len - len(message))
        logger.info('\r' + message + padding)
