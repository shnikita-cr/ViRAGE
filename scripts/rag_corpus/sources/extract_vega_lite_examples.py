...
def extract_vega_lite_examples(input_dir: Path) -> list[SourceRecord]:
    ...
    for path in sorted(input_dir.rglob("*.json")):
        ...
        text_parts = [
            f"Title: {title}",
            f"Chart mark/family: {mark}" if mark else "",
            f"Encoding roles: {'; '.join(encoding)}" if encoding else "",
            compact_text(spec.get("description")),
            "Use this only as source material for rule extraction, not as a runtime Vega-Lite template.",
        ]
        text = compact_text(". ".join(part for part in text_parts if part), max_chars=4000)
        records.append(SourceRecord(
            record_id=f"vega_lite__{stable_hash(str(path.relative_to(input_dir)))}",
            source_dataset="vega_lite_examples",
            source_path=str(path),
            source_type="vega_lite_example_summary",
            title=title,
            text=text,
            chart_family=mark,
            metadata={
                "file_name": path.name,
                "relative_source_path": str(path.relative_to(input_dir)),
                "encoding_summary": encoding,
                "contains_spec_in_raw_only": True,
            },
            raw={"title": title, "mark": mark, "encoding_summary": encoding},
        ))
    return records
