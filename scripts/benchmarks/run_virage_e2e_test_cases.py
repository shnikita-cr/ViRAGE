...
from src.benchmark.progress import BenchmarkStatusBar
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    str(row.get("case_id", "")),
                    str(row.get("suite", "")),
                    str(row.get("input_modality", "")),
                    str(row.get("analysis_task", "")),
                    str(row.get("output_target", "")),
                    str(row.get("status", "")),
                    str(row.get("subrun_count", "")),
                    _format_float(row.get("valid_spec_rate")),
                    _format_float(row.get("empty_chart_rate")),
                    _format_float(row.get("mean_spec_score")),
                    _format_float(row.get("mean_vision_score")),
                    _format_float(row.get("mean_publication_layout_score")),
                    str(row.get("run_dir", "")),
                ]
            )
            + " |"
    ...
    context.update(
        {
            "benchmark_case": {
                ...
                "query_specificity": case.query_specificity,
...
def run_case(
    ...
    *,
    ...
    parent_run_id: str,
    execute: bool,
    image_folder_override: str | None,
) -> dict[str, Any]:
    ...
    args = [
        ...
        data_path,
        "--input-type",
        _case_input_type(case),
        "--run-id",
        case_run_id,
        "--user-context-json",
        json.dumps(_case_user_context(case), ensure_ascii=False),
    ]
    if execute:
        args.append("--execute")
    ...
    result: dict[str, Any] = {
        **case.model_dump(),
        ...
        "chart_plan_path": chart_plan_path.as_posix() if chart_plan_path.exists() else "",
        "orchestrator_report_path": orchestrator_report_path.as_posix() if orchestrator_report_path.exists() else "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.list_suites:
        print(json.dumps(suites_manifest, ensure_ascii=False, indent=2))
        return 0
    ...
    request = {
        ...
        "suites_file": str(args.suites_file),
        "selected_suites": sorted(selected_suites or []),
        ...
        json.dumps(
            {
                ...
                "benchmark_report": (report_dir / "benchmark_report.md").as_posix(),
                "per_case_results": (report_dir / "per_case_results.csv").as_posix(),
            },
            ensure_ascii=False,
            indent=2,
        )
