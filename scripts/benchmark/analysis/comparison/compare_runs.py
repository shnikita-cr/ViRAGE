...
ROOT = Path('.').resolve()
_NUMERIC_METRICS = ('spec_score', 'vision_score', 'duration_seconds', 'prompt_tokens', 'completion_tokens', 'total_tokens')
_RESULT_FILE_CANDIDATES = ('benchmark_report.json', 'analysis_benchmark_report.json')
...
def main() -> None:
    ...
    logger.info(f"Common cases: {comparison['summary']['common_cases']}")
    logger.info(f"Improved: {comparison['summary']['improved_cases']}")
    logger.info(f"Degraded: {comparison['summary']['degraded_cases']}")
    logger.info(f'Report directory: {output.resolve()}')
...
def compare_runs(left: dict, right: dict, *, bootstrap_samples: int) -> dict:
    ...
    for case_id in common_ids:
        ...
        if isinstance(quality_delta, (int, float)):
            if quality_delta > 1e-09:
                improved += 1
            elif quality_delta < -1e-09:
                degraded += 1
            else:
                unchanged += 1
    ...
    return {'summary': {'left_cases': len(left_items), 'right_cases': len(right_items), 'common_cases': len(common_ids), 'missing_in_left': sorted(set(right_items) - set(left_items)), 'missing_in_right': sorted(set(left_items) - set(right_items)), 'improved_cases': improved, 'degraded_cases': degraded, 'unchanged_cases': unchanged, 'fixed_by_right': fixed, 'broken_by_right': broken}, 'left_manifest': left.get('manifest'), 'right_manifest': right.get('manifest'), 'metric_deltas': metric_deltas, 'rows': rows}

def _compare_case(case_id: str, left: dict, right: dict) -> dict:
    row = {'case_id': case_id, 'left_error': left.get('error'), 'right_error': right.get('error'), 'left_failed': _is_failed(left), 'right_failed': _is_failed(right)}
...
def _aggregate_metric_deltas(rows: list[dict], *, bootstrap_samples: int) -> dict:
    ...
    for key in sorted({key for row in rows for key in row if key.startswith('delta_')}):
        ...
        if not values:
            continue
        out[key] = {'mean': round(sum(values) / len(values), 6), 'median': round(statistics.median(values), 6), 'ci95': _bootstrap_ci(values, samples=bootstrap_samples)}
    return out
...
def _load_run(path: Path) -> dict:
    ...
    return {'report': report, 'results': report.get('results', []), 'manifest': json.loads(manifest_path.read_text(encoding='utf-8')) if manifest_path.exists() else None}
...
def _write_outputs(comparison: dict, output: Path) -> None:
    ...
    lines = ['# Benchmark comparison', '', f"Common cases: {comparison['summary']['common_cases']}", f"Improved cases: {comparison['summary']['improved_cases']}", f"Degraded cases: {comparison['summary']['degraded_cases']}", f"Fixed by right: {comparison['summary']['fixed_by_right']}", f"Broken by right: {comparison['summary']['broken_by_right']}", '', '## Metric deltas', '']
