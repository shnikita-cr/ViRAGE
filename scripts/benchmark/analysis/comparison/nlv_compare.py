...
import pandas as pd
DEFAULT_METRICS = ['total_cases', 'successful_cases', 'failed_cases', 'visualization_error_rate', 'empty_chart_rate', 'mean_spec_score', 'mean_spec_score_failure_as_zero', 'mean_vision_score', 'mean_vision_score_failure_as_zero', 'median_spec_score', 'median_vision_score', 'mean_duration_seconds', 'p95_duration_seconds', 'mean_prompt_tokens', 'mean_completion_tokens', 'mean_total_tokens', 'total_tokens', 'chart_text_consistency_rate']
...
def build_dataframe(no_rag: dict[str, Any], rag: dict[str, Any], metrics: list[str]) -> pd.DataFrame:
    ...
    for metric in metrics:
        ...
        a_num = _number(a)
        b_num = _number(b)
        rows.append({'metric': metric, 'no_rag': a, 'rag': b, 'delta_rag_minus_no_rag': None if a_num is None or b_num is None else round(float(b_num) - float(a_num), 6)})
