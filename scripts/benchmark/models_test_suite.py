...
DEFAULT_LOCAL_ALL_IN_ONE_MODELS: tuple[str, ...] = (
    "qwen3.5:latest",
    "qwen3.5:4b",
    "gemma3:4b",
    "qwen2.5vl:latest",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--runner", default="scripts/benchmark/run_virage_e2e_test_cases.py")
...
def build_suite_definition(
    ...
    return SuiteDefinition(
        ...
        case_count=int(case_counts.get(suite_name, 0)),
    )
...
def run_benchmark(
    ...
    return RunResult(
        model=extract_model_from_run_id(run_id),
        suite=suite.name,
        run_id=run_id,
...
def print_plan(models: Sequence[str], suites: Sequence[SuiteDefinition]) -> None:
    print("Selected local all-in-one models:")
    for model in models:
        print(f"  - {model}")
...
def print_result(result: RunResult) -> None:
    print(f"{result.status.upper()}: model={result.model} suite={result.suite} exit_code={result.exit_code}")
    print(f"  stdout: {result.stdout_path.as_posix()}")
    print(f"  stderr: {result.stderr_path.as_posix()}")
...
def main() -> int:
    ...
    for model in models:
        ...
        for suite in suites:
            ...
            result = run_benchmark(runner, config_path, suite, run_id, log_dir)
