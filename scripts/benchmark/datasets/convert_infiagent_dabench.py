...
def main() -> None:
    ...
    parser.add_argument('--chart-answerable-only', action='store_true', help='Write only cases that can reasonably be answered from a static chart image.')
    args = parser.parse_args()
    cases = convert_da_agent_dataset(source_root=args.source_root, output_path=args.output, include_constraints=not args.no_constraints, include_format=not args.no_format, dataset_name=args.dataset_name, chart_answerable_only=args.chart_answerable_only)
    logger.info(f'Converted cases: {len(cases)}')
    logger.info(f'Output: {Path(args.output).resolve()}')
    logger.info(f"Manifest: {Path(args.output).with_suffix('.manifest.json').resolve()}")
