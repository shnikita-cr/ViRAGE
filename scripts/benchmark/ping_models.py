...
OLLAMA_CHAT_MODELS: Mapping[str, Sequence[str]] = {
...
IMAGE_CAPABLE_OLLAMA_MARKERS: Sequence[str] = (
    "vl",
    "minicpm",
    "moondream",
    "gemma3",
...
def failed_result(category: str, model: str, error: BaseException) -> PingResult:
    return PingResult(
        ...
        status="failed",
        details=f"{error.__class__.__name__}: {str(error)[:260]}",
    )


def print_torch_status(device: str) -> None:
    print(f"PyTorch: {torch.__version__}")
    print(f"Requested HF device: {device}")
...
def print_result(result: PingResult) -> None:
    ...
    print(
        f"{result.category:<12} | {result.model:<42} | "
        f"{latency:<10} | {throughput:<10} | {result.status:<8} | {result.details}"
    )
...
def print_header() -> None:
    print(f"{'Тип':<12} | {'Модель':<42} | {'Ping':<10} | {'Ток/сек':<10} | {'Статус':<8} | Детали")
    print("-" * 126)
