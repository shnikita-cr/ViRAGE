...
def main() -> None:
    ...
    for run in runs:
        write_all_in_one_config(
            base_config=args.base_config.resolve(),
            output_config=run.config_path,
            model=run.model,
        )
        print(f"\n=== NLV model={run.model} ===", flush=True)
...
def build_model_runs(*, models: Sequence[str], output_root: Path) -> list[ModelRun]:
    return [
        ModelRun(
            ...
            config_path=output_root / "configs" / f"local_all_in_one_{slugify_model_name(model)}.toml",
            output_dir=output_root / f"local_all_in_one_{slugify_model_name(model)}",
            logs_dir=output_root / f"local_all_in_one_{slugify_model_name(model)}" / "logs",
        )
        for model in models
    ]
...
def print_run_result(*, run: ModelRun, result: ProcessResult) -> None:
    ...
    print(f"{status}: model={run.model} output={run.output_dir.as_posix()}", flush=True)
    if result.exit_code != 0:
        print(f"stderr: {result.stderr_path.as_posix()}", flush=True)
