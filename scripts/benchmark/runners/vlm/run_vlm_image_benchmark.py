...
SCORE_FIELDS = ['non_empty_score', 'readability_score', 'label_quality_score', 'legend_quality_score', 'visual_overload_score', 'plot_area_usage_score', 'axis_domain_score', 'layout_compactness_score', 'repeat_axis_label_score', 'publication_layout_score', 'overall_visual_score']

def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Evaluate a folder of chart images with ViRAGE VLM image-only visual quality benchmark.')
    ...
    parser.add_argument('--publication-threshold', type=float, default=0.7, help='Threshold for publication pass/fail based on overall and publication layout scores.')
...
def iter_image_paths(input_dir: str | Path, *, max_images: int | None=None) -> list[Path]:
    ...
    paths = sorted((path for path in root.rglob('*') if path.is_file() and path.suffix.lower() in SUPPORTED_IMAGE_EXTENSIONS))
...
def summarize_results(*, run_id: str, input_dir: str, results: list[VLMImageBenchmarkImageResult], publication_threshold: float, output_files: dict[str, str] | None=None) -> VLMImageBenchmarkSummary:
    ...
    for item in ok_results:
        if item.result is None:
            raise ValueError('Successful benchmark item has no parsed result.')
...
def _write_csv(path: Path, results: list[VLMImageBenchmarkImageResult]) -> None:
    ...
    fieldnames = ['image_path', 'relative_path', 'file_name', 'status', 'error', 'passed_publication_threshold', 'detected_chart_type', 'is_chart_image', 'is_blank_or_unreadable', *SCORE_FIELDS, 'confidence', 'detected_issues', 'recommendations', 'chart_description']
    with path.open('w', encoding='utf-8', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for item in results:
            result = item.result
            row: dict[str, Any] = {'image_path': item.image_path, 'relative_path': item.relative_path, 'file_name': item.file_name, 'status': item.status, 'error': item.error, 'passed_publication_threshold': item.passed_publication_threshold}
            if result is not None:
                payload = result.model_dump()
                row.update({field: payload.get(field) for field in SCORE_FIELDS})
                row.update({'detected_chart_type': result.detected_chart_type, 'is_chart_image': result.is_chart_image, 'is_blank_or_unreadable': result.is_blank_or_unreadable, 'confidence': result.confidence, 'detected_issues': ' | '.join(result.detected_issues), 'recommendations': ' | '.join(result.recommendations), 'chart_description': result.chart_description})
            writer.writerow(row)

def _write_report(path: Path, summary: VLMImageBenchmarkSummary) -> None:
    lines = ['# ViRAGE image-only VLM benchmark report', '', 'This benchmark evaluates only chart images. It does not use source tables, user queries, Vega-Lite specs, or ground truth.', '', f'Run ID: `{summary.run_id}`', f'Input folder: `{summary.input_dir}`', f'Total images: **{summary.total_images}**', f'Evaluated images: **{summary.evaluated_images}**', f'Failed images: **{summary.failed_images}**', f'Publication threshold: **{summary.publication_threshold}**', f'Publication pass rate: **{summary.pass_rate_publication_threshold}**', '', '| Metric | Value |', '|---|---:|', f'| mean_overall_visual_score | {summary.mean_overall_visual_score} |', f'| median_overall_visual_score | {summary.median_overall_visual_score} |', f'| std_overall_visual_score | {summary.std_overall_visual_score} |', f'| mean_non_empty_score | {summary.mean_non_empty_score} |', f'| mean_readability_score | {summary.mean_readability_score} |', f'| mean_plot_area_usage_score | {summary.mean_plot_area_usage_score} |', f'| mean_axis_domain_score | {summary.mean_axis_domain_score} |', f'| mean_layout_compactness_score | {summary.mean_layout_compactness_score} |', f'| mean_repeat_axis_label_score | {summary.mean_repeat_axis_label_score} |', f'| mean_publication_layout_score | {summary.mean_publication_layout_score} |', '', '## Most frequent issues', '']
...
def main(argv: list[str] | None=None) -> int:
    ...
    ok_count = 0
    error_count = 0
    progress = ConsoleProgressBar(total=len(image_paths), title='VLM image benchmark', enabled=not args.no_progress)
    for index, image_path in enumerate(image_paths, start=1):
        relative_path = image_path.relative_to(root).as_posix()
        progress.update(index - 1, ok=ok_count, errors=error_count, stage='vlm_judge', label=relative_path)
        try:
            ...
            results.append(VLMImageBenchmarkImageResult(image_path=image_path.as_posix(), relative_path=relative_path, file_name=image_path.name, status='ok', result=result, passed_publication_threshold=passed))
            ok_count += 1
        except (RuntimeError, ValueError, TypeError, OSError, KeyError, IndexError, AttributeError, ImportError) as exc:
            results.append(VLMImageBenchmarkImageResult(image_path=image_path.as_posix(), relative_path=relative_path, file_name=image_path.name, status='failed', error=f'{type(exc).__name__}: {exc}'))
            error_count += 1
        progress.update(index, ok=ok_count, errors=error_count, stage='vlm_judge', label=relative_path)
