...
from src.domain.models import EmptyChartCheckResult, InsightVerificationResult, StructuralSpecMetric, \
    VisualQualityMetric
...
def test_evaluation_summary_service_aggregates_metrics() -> None:
    ...
    result = service.invoke(
        ...
        empty_chart_check=EmptyChartCheckResult(empty_chart_signal=False, non_empty_render=True,
                                                empty_chart_status="non_empty"),
        insight_verification=InsightVerificationResult(verified_insights=["Insight"], rejected_claims=[],
                                                       insight_verification_summary="ok", all_verified=True),
    )
