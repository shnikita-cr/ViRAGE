...
def test_presentation_consistency_uses_repeat_safe_labels() -> None:
    ...
    assert inner["encoding"]["y"]["axis"]["title"] == "Average Value of metric named in panel header"
    assert "title" not in inner["encoding"]["y"]
    assert inner["encoding"]["tooltip"][0]["title"] == "Average Value of metric named in panel header"
...
def test_presentation_consistency_uses_repeat_distribution_title_for_repeat_boxplots() -> None:
    ...
    assert result.spec["spec"]["encoding"]["y"]["axis"]["title"] == "Panel metric value"
